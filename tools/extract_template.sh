...
soft='GLPI - viewruleticket plugin'
version='1.1.0'
...
WORKING_DIR=$(readlink -f "templates/*") # Script will be executed from "vendor/bin" directory
...
do
    ...
    sed -i -r "s|⁨standard input⁩:([0-9]+)|`echo $filename | sed "s|./||"`:\1|g" locales/glpi.pot
done
